...
def visualize_contour_components(quantized, K):
    h, w = quantized.shape
    canvas = np.zeros((h, w, 3), dtype=np.uint8)

    for k in range(1, K+1):
        mask = (quantized == k).astype(np.uint8)
        ...
        canvas[best_comp > 0] = color

    canvas_up = cv2.resize(canvas, None, fx=3, fy=3, interpolation=cv2.INTER_NEAREST)

    cv2.imshow("Contour Components", canvas_up)

def visualize_contour_structure(quantized, K):
    h, w = quantized.shape
    canvas = np.zeros((h, w, 3), dtype=np.uint8)
    ...
    for k in range(1, K+1):
        ...
        ys, xs = np.where(best_comp > 0)
        cx, cy = int(np.mean(xs)), int(np.mean(ys))

        centroids.append((cx, cy))

        # 영역 표시
        canvas[best_comp > 0] = (100, 100, 100)
    ...
    for i in range(1, len(centroids)):
        cv2.line(canvas, centroids[i-1], centroids[i], (0, 255, 255), 2)
        
    canvas_up = cv2.resize(canvas, None, fx=3, fy=3, interpolation=cv2.INTER_NEAREST)
    cv2.imshow("Contour Structure", canvas_up)

def visualize_contour_levels(d_norm, quantized):
    vis = (quantized.astype(np.float32) / quantized.max() * 255).astype(np.uint8)
    vis = cv2.applyColorMap(vis, cv2.COLORMAP_JET)

    vis_up = cv2.resize(vis, None, fx=3, fy=3, interpolation=cv2.INTER_NEAREST)

    cv2.imshow("Contour Levels", vis_up)
...
def build_corner_pointcloud_from_two_directions(x0, z0, dx1, dz1, dx2, dz2, length=5.0, n=100):

    pts = []

    # 첫 번째 벽
    for t in np.linspace(-length, length, n):
        x = x0 + dx1 * t
        z = z0 + dz1 * t
        pts.append([x, z])
...
def depth_saliency_thread():
    ...
    while True:
        ...
        if segmentation_block and edge_block and depth_block:
            ...
            # 🔥 1️⃣ depth normalize (0~1)
            d = structure_depth.copy()
            valid = d > 0
            ...
            if np.sum(valid) < 100:
                ...
                # 🔥 2️⃣ level quantization (등고선)
                K = 6
                bins = np.linspace(0, 1, K+1)
                quantized = np.digitize(d_norm, bins)

                centroids = []
                areas = []

                # 🔥 3️⃣ 각 level에서 대표 component 추출
                for k in range(1, K+1):

                    level_mask = (quantized == k).astype(np.uint8)

                    if np.sum(level_mask) < 50:
                        continue

                    # connected component
                    num_labels, labels_im = cv2.connectedComponents(level_mask)
                    ...
                    if best_comp is None or max_area < 50:
                        continue
                ...
                if len(centroids) >= 3:
                    ...
                    decreasing_score = np.sum(area_diff < 0) / len(area_diff)
                    ...
                    convergence_score = np.sum(dist_diff < 0) / len(dist_diff)

                    # 👉 (3) peak 위치 (가장 먼 영역)
                    peak_idx = np.argmax(areas)
                    peak_pos = centroids[peak_idx]

                    # 중심에서 멀리 있으면 wall 가능성 ↑
                    h, w = d.shape
                    img_center = np.array([w/2, h/2])
                    peak_dist = np.linalg.norm(peak_pos - img_center) / np.linalg.norm([w, h])

                    peak_score = 1.0 - peak_dist  # 중앙에 있을수록 corner
                    ...
                    # 오차 비교
                    err1 = np.mean((y - y_fit1)**2)
                    err2 = np.mean((y - y_fit2)**2)

                    # 🔥 곡률 score
                    curvature_score = (err1 - err2) / (err1 + 1e-6)
            ...
            if segmentation_vis:
                ...
                visualize_contour_levels(d_norm, quantized)
                visualize_contour_components(quantized, K)
                visualize_contour_structure(quantized, K)
            ...
            if segmentation_vis:  
                ...
                print("corner confidence:", corner_conf)

        if pointcloud_block:
            
            depth_corrected = 1.0 / (smoothed + 1e-6)

            cross_section = extract_depth_cross_section(depth_corrected)
            ...
            if structure != "UNKNOWN":
                ...
                # 🔥 quantized에서 가장 먼 level부터 사용 (peak 근처)
                levels = list(range(K, 0, -1))

                for k in levels:

                    level_mask = (quantized == k)

                    if np.sum(level_mask) < 100:
                        continue

                    # connected component 중 largest
                    num_labels, labels_im = cv2.connectedComponents(level_mask.astype(np.uint8))
                    ...
                    if best_comp is None or max_area < 100:
                        continue

                    # 🔥 이걸 candidate로 사용
                    component_mask = best_comp

                    # 🔥 slope, var 계산 추가 (필수)
                    slope, var = compute_metric_slope(
                ...
                if len(candidates) == 0:
                    ...
                    if structure == "WALL":
                        ...
                        # 🔥 mask 기반으로 필터링
                        mask_indices = best["mask"].flatten()
                        ...
                        for y in range(h):
                            for x in range(w):
                                if best["mask"][y, x]:
                                    z = depth_corrected[y, x]
                                    X, Z = pixel_to_camera_coords(np.array([x]), np.array([z]), w)
                                    filtered_points.append([X[0], Z[0]])

                        filtered_points = np.array(filtered_points)

                        model, inliers = fit_line_ransac(filtered_points)
                        ...
                        if len(pts) < 20:
                            ...
                            for i in range(2, len(pts)-2):
                                pts_smooth[i] = np.mean(pts[i-2:i+3], axis=0)

                            # 🔥 2️⃣ curvature
                            v1 = pts_smooth[1:-1] - pts_smooth[:-2]
                            v2 = pts_smooth[2:] - pts_smooth[1:-1]
                            ...
                            corner = pts_smooth[corner_idx]

                            # 🔥 4️⃣ dominant line (전체에서 RANSAC 1번만)
                            model, inliers = fit_line_ransac(pts)

                            if model is None or len(inliers) < 10:
                                pointcloud = None

                            else:
                                # 🔥 방향 벡터
                                d = np.array([-model[1], model[0]])
                                d /= (np.linalg.norm(d) + 1e-6)

                                # 🔥 직교 방향 (핵심)
                                d_perp = np.array([-d[1], d[0]])

                                # 🔥 5️⃣ outward 방향 결정 (곡선 기준)
                                vecs = pts_smooth - corner

                                if np.sum((vecs @ d) > 0) < np.sum((vecs @ d) < 0):
                                    d = -d

                                if np.sum((vecs @ d_perp) > 0) < np.sum((vecs @ d_perp) < 0):
                                    d_perp = -d_perp

                                # 🔥 6️⃣ 길이 자동 추정 (곡선 외곽 기반)
                                proj_d = vecs @ d
                                proj_perp = vecs @ d_perp

                                length1 = np.percentile(np.abs(proj_d), 90)
                                length2 = np.percentile(np.abs(proj_perp), 90)

                                # 최소 길이 보정
                                length1 = max(length1, 0.5)
                                length2 = max(length2, 0.5)

                                # 🔥 7️⃣ L-shape 생성
                                t1 = np.linspace(0, length1, 100)
                                t2 = np.linspace(0, length2, 100)

                                line1 = corner + np.outer(t1, d)
                                line2 = corner + np.outer(t2, d_perp)

                                pointcloud = np.vstack([line1, line2])
